...
enum {
    ...
    /// @note  DB_EV_SELCHANGED is obsolete and isn't emitted; DB_EV_PLAYLISTCHANGED with DDB_PLAYLIST_CHANGE_SELECTION should be used instead
    DB_EV_SELCHANGED = 22,
...
typedef struct {
    ...
    /// save all playlists
    /// @note Remember to call `plt_modified` on playlists which need saving.
    /// @note See more information near the `plt_modified` declaration
    ...
    */
    ...
    /// @returns Returns -1 on failure, output size on success
...
enum {
    ...
    /// @deprecated deprecated since API 1.5, use DB_ACTION_MULTIPLE_TRACKS instead
